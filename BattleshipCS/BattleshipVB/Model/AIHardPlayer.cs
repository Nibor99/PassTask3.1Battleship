...
namespace BattleshipVB
{
    ...
    /// <param name="column">the column that will be shot at</param>
        protected override void GenerateCoords(ref int row, ref int column)
        {
            ...
            while (row < 0 || column < 0 || row >= EnemyGrid.Height || column >= EnemyGrid.Width || (EnemyGrid as SeaGrid).get_Item(row, column)  != TileView.Sea);
    ...
    /// <param name="column">the column of the targets location</param>
        ...
        {
            if (row >= 0 && column >= 0 && row < EnemyGrid.Height && column < EnemyGrid.Width && (EnemyGrid as SeaGrid).get_Item(row, column) == TileView.Sea)
